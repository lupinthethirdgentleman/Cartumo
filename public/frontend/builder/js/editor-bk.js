...
$('#ex1').slider({
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if ( replace_container ) {
                            ...
                            if ( $(element_handler).hasClass('add-grid-in-row') ) {
    ...
    $(document).on('click', '#icon_list_setting_save', function(e) {
        ...
        //headline text
        //alert(json.hid_icon_code);
        $(tmp_setting_element).find('.icon-list').attr('data-icon-type', json.hid_icon_class);
        $(tmp_setting_element).find('.icon-list > li').each(function(index, element) {
            $(element).attr('data-before', "\\" + json.hid_icon_code); //anything is the 'content' value
        });
...
$(document).on('click', '.open-order-two-step-settings', function(e) {
    ...
    /*$(document).on('click', '.open-advance-button-setings-modal', function(e) {
				...
				alert($("#frm_advance_button_settings").attr('data-load-poduct-action'));
                
                //load products based on the funnel type
                $.ajax({
                    type: 'get',
                    url: $("#frm_advance_button_settings").attr('data-load-poduct-action') + '/funnels/' + $("#hid_funnel_id").val() + '/load-products/',
    ...
    $(document).on('change', '#product_add_product, #advance_button_add_product', function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: 'https://cartumo.io/funnels/' + $("#hid_funnel_id").val() + '/product/' + $(this).val() + '/variants',
            data: '_token=' + $("#csrf_token").val() + '&step_id=' + $("#hid_funnel_step_id").val() + '&page_id=' + $("#hid_page_id").val(),
            beforeSend: function() {
                //$(element_products).after('<span id="product_loading">loading...</span>');
            },
    ...
    $(document).on("change", "#product_add_variant_container select, #advance_button_variant_container select", function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: "https://cartumo.io/product/" + product_id + "/variant/details",
    ...
    $(document).on('click', '.open-icon-list-setings-modal', function(e) {
        ...
        //alert(list_item_count);

        for ( i=1; i<=list_item_count; i++ ) {

            var element = $(list_items).get(i-1);
            var list_icon = $(element).find('span > i').attr('class').split(' ');
            var list_content = $(element).text();

            //alert(list_icon[1] + ', ' + list_content);
            //<input type="hidden" name="hid_selected_icon" value="' + list_icon[1] + '" />
            var html = '<div class="form-group"><div class="row clearfix"><div class="col-md-2"><button type="button" class="btn btn-default btn-block btn-lg" data-iconset="fontawesome" data-icon="' + list_icon[1] + '" role="iconpicker">ss</button> </div> <div class="col-md-10"> <input type="text" name="list_text" class="form-control" value="' + list_content + '" /> </div> </div></div>';


            $(container).append(html);
            $(container).find('.form-group:last-child button').iconpicker({
            ...
            $('#icon_list_color').val($(option_open_for).find('.wrapper > .icon-list > li > span > i').prop('style')['color']);
            //alert($(html).find('button').html());
            //$(html).find('button').append('<input type="hidden" name="hid_selected_icon" value="' + list_icon[1] + '" />');
        }
    });

    var icon_container_button;
    $(document).on('click', '.repeat-icon-list .iconpicker', function(e) {
    ...
    $(document).on("click", ".iconpicker-popover .table-icons tr > td > button", function(e) {
        ...
        $(icon_container_button).append("<input type='hidden' name='hid_selected_icon' value='" + $(this).attr('value') + "' />");
    });
    ...
    $(document).on('click', '#icon_list_add_item', function(e) {
        ...
        var html = '<div class="form-group"><div class="row clearfix"><div class="col-md-2"><button type="button" class="btn btn-default btn-block btn-lg" data-iconset="fontawesome" data-icon="fa-check" role="iconpicker"><input type="hidden" name="hid_selected_icon" value="fa-check" /></button> </div> <div class="col-md-10"> <input type="text" name="list_text" class="form-control"/> </div> </div></div>';
    ...
    $(document).on('click', '.open-paragraph-setings-modal', function(e) {
        ...
        if ( $(option_open_for).find('.wrapper > p').attr('style') ) {
            ...
            $("#paragraph_text_color").val(rgb2hex($(option_open_for).find('.wrapper > p').prop('style')['color']));
            $("#paragraph_bg_color").val(rgb2hex($(option_open_for).find('.wrapper > p').prop('style')['background-color']));
        }
    ...
    $(document).on('click', '.open-text-block-settings', function(e) {
        ...
        $('#text_clock_html_editor').summernote('code', $(option_open_for).find('.wrapper > div').html());
    ...
    $(document).on('click', '#icon_list_setting_save', function(e) {
        ...
        for ( i=0; i<json.list_text.length; i++ ) {
            html += '<li><span><i class="fa ' + json.hid_selected_icon[i] + '" aria-hidden="true"></i></span> ' + json.list_text[i] + '</li>';
        }

        $(option_open_for).find('.wrapper > ul').html(html);
        $(option_open_for).find('.wrapper > ul > li > span > i').css('color', json.icon_list_color);
    ...
    $(document).on('click', '#paragraph_setting_save', function(e) {
        ...
        $(option_open_for).find('.wrapper > p').css('color', json.paragraph_text_color);
        $(option_open_for).find('.wrapper > p').css('background-color', json.paragraph_bg_color);
    ...
    $(document).on('click', '#text_block_setting_save', function(e) {
        ...
        $(option_open_for).find('.wrapper > div').html(json.sub_headline_text);
